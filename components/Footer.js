    ...
    <footer className="footer">
      <div className="wrap">
        ...
        <div style={{ display: 'flex', gap: 20, justifyContent: 'center', marginTop: 12, flexWrap: 'wrap' }}>
          <Link href="/privacy" style={{ color: '#444', fontSize: 12, textDecoration: 'none' }}>{lang === 'en' ? 'Privacy Policy' : '개인정보처리방침'}</Link>
          <Link href="/terms" style={{ color: '#444', fontSize: 12, textDecoration: 'none' }}>{lang === 'en' ? 'Terms of Use' : '이용약관'}</Link>
          <Link href="/faq" style={{ color: '#444', fontSize: 12, textDecoration: 'none' }}>FAQ</Link>
          <Link href="/blog" style={{ color: '#444', fontSize: 12, textDecoration: 'none' }}>{lang === 'en' ? 'Blog' : '블로그'}</Link>
        </div>
