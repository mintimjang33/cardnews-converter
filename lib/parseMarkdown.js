 ...
 *  - iframe 인라인 (유튜브 영상 삽입 등, 태그 그대로 통과)
...
export function sanitizeHtml(html) {
  ...
  // DOMPurify 없을 때 — script/이벤트핸들러만 제거
  // ⚠️ iframe은 관리자가 직접 삽입하는 유튜브 영상 임베드 용도라 여기서 걸러내지 않는다
  // (걸러내면 admin 미리보기/실제 블로그 페이지 모두에서 영상이 영영 렌더링되지 않음).
  return html
...
export function parseMarkdown(md) {
  ...
  // 1b) iframe 블록 분리 보존 (영상 삽입 — 마크다운 변환 중 깨지지 않게)
