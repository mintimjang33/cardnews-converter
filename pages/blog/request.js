...
function WriteForm({ onClose, onSuccess }) {
  ...
  const [form, setForm] = useState({ title: '', content: '', author_name: '', contact: '', is_secret: false, secret_password: '' })
  const [saving, setSaving] = useState(false)
  const [error, setError] = useState('')

  const handleSecretToggle = (val) => {
    if (val && !warningConfirmed) { setShowWarning(true); return }
  ...
  const handleSubmit = async () => {
    ...
    try {
      const content = form.contact.trim()
        ? `${form.content.trim()}\n\n📧 연락처: ${form.contact.trim()}`
        : form.content.trim()
  ...
  return (
    <>
      {showWarning && (
        <SecretWarningModal
          onConfirm={() => { setShowWarning(false); setWarningConfirmed(true); setForm(f => ({ ...f, is_secret: true })) }}
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: 16, fontWeight: 800, marginBottom: 20, color: '#111' }}>📬 서비스 요청하기</h3>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
            placeholder="원하시는 기능/서비스 제목" maxLength={100}
            style={{ padding: '11px 14px', borderRadius: 10, border: '1.5px solid #e5e7eb', fontSize: 15, outline: 'none' }} />
          <textarea value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))}
            placeholder="어떤 서비스가 필요하신지 자세히 적어주세요 :)" rows={6} maxLength={2000}
          ...
          <input value={form.contact} onChange={e => setForm(f => ({ ...f, contact: e.target.value }))}
            placeholder="연락처 (선택 — 이메일, SNS 등 답변 받을 곳)" maxLength={100}
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            {form.is_secret && (
              <input
                ...
                placeholder="비밀번호 4자리" maxLength={4}
...
function PostDetail({ post, onClose }) {
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 12, fontSize: 13, color: '#888', marginBottom: 20 }}>
...
export default function RequestPage() {
  ...
  const fetchPosts = useCallback(async (offset, append) => {
    ...
    try {
      const res = await fetch(`/api/blog/posts?post_type=request&limit=${PAGE_SIZE}&offset=${offset}`)
      const data = await res.json()
  ...
  return (
    <div className="light-theme">
      <Head>
        <title>부탁해요 - Down Tools</title>
        <meta name="description" content="원하는 서비스나 기능을 요청해 주세요!" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>

      <Header siteName="부탁해요" siteHref="/blog/request" />
      ...
      <div className="wrap" style={{ paddingTop: 40, paddingBottom: 60 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 12 }}>
          <div>
            <h1 style={{ fontSize: 26, fontWeight: 900, color: '#111' }}>📬 부탁해요~</h1>
            <p style={{ color: '#888', fontSize: 14, marginTop: 4 }}>원하는 서비스나 기능을 요청해 주세요. 최대한 반영할게요!</p>
          </div>
        ...
        <div style={{ marginTop: 28 }}>
          {showWrite && !selectedPost && (
            <WriteForm onClose={() => setShowWrite(false)} onSuccess={() => { setShowWrite(false); fetchPosts(0, false) }} />
          )}
            ...
            <>
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '60px 20px', color: '#aaa' }}>
                  ...
                  <p>아직 요청이 없어요. 원하는 서비스를 요청해 보세요!</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  ...
                  {posts.map(post => (
                    <div key={post.id}
                      onClick={() => post.is_secret ? setPwModal(post) : setSelectedPost(post)}
                      style={{
                        display: 'grid', gridTemplateColumns: '1fr 80px 100px',
