...
export default function BlogAdminPage() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('admin_token')) { setAuthed(true); loadPosts(); loadCategories() }
  }, [])
  ...
  const doLogin = async () => {
    ...
    setAuthed(true); loadPosts(); loadCategories()
  }
  ...
  if (!authed) return (
    <div style={{ ...S.page, display:'flex', alignItems:'center', justifyContent:'center' }}>
      <Head><title>블로그 어드민</title></Head>
      <div style={{ background:'#161616', border:'1px solid #2a2a2a', borderRadius:14, padding:40, width:340 }}>
        <h2 style={{ fontSize:20, fontWeight:800, marginBottom:20 }}>🔐 블로그 어드민 로그인</h2>
        <input type="password" placeholder="비밀번호" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==='Enter'&&doLogin()} style={{ ...S.input, marginBottom:10 }} />
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ background:'#161616', borderBottom:'1px solid #2a2a2a', padding:'16px 0', position:'sticky', top:0, zIndex:100 }}>
        <div style={{ ...S.wrap, display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:12 }}>
            ...
            <span style={{ fontWeight:800, fontSize:16 }}>📝 블로그 어드민</span>
          </div>
          <div style={{ display:'flex', gap:8, alignItems:'center', flexWrap:'wrap' }}>
            {view==='list' && (
              <>
                <Link href="/blog-menu" style={{ ...S.btn('#333'), padding:'8px 14px', fontSize:13, textDecoration:'none' }}>📂 메뉴 관리</Link>
            ...
            <button onClick={()=>{sessionStorage.removeItem('admin_token');setAuthed(false)}} style={{ ...S.btn('#222'), padding:'8px 12px', fontSize:13 }}>로그아웃</button>
          </div>
        </div>
      </div>
